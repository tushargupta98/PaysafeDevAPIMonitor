...
/**
 * This class instantiates the business processing for all the requests  made by the exposed services.
 * @author Tushar
 * @version 1.0
 */
public class MonitoringBO {
	/**
	 * this dynamically provides the type of persistence/data-management mechanism should be used. Makes use of Factory Design pattern.
	 * this object provides access to the list of all the services/servers monitored by the application and their current states
	 */
	private ServiceMonitorDAO smDAO = MonitoringFactory.getServiceMonitorDAO(ApplicationConstants.SERVICE_MONITOR_DAO);
	/**
	 * this dynamically provides the type of persistence/data-management mechanism should be used. Makes use of Factory Design pattern.
	 * this object provides access to all the status logged in for all the services/servers at a given instance.
	 * The monitoring for a particular service may or may not have stopped but it provides log since most recent monitoring-start 
	 */
	...
	/**
	 * this method invokes the creation of a Monitor for a particular service/server.
	 * it first validates the time interval, it should be minimum of 1 second.
	 * it then checks if a monitor already exists for a particular service/server.
	 * if exists  and is currently monitoring then informs return the status the the monitor 
	 * already exists and does not create a new monitor,
	 * otherwise it starts the monitoring of the service/server
	 * returns an appropriate message in terms of ExecutionResponseVO object to the calling function
	 * This method has been made synchronized because the process involves creation of a monitoring thread per service/server
	 * @author Tushar
	 * @param srVO
	...
	/**
	 * this method tries to stop the Monitor for a particular service/server.
	 * it takes in the url of the service/server.
	 * it then checks if a monitor already exists for a particular service/server.
	 * if exists  and is currently monitoring then it stop the corresponding monitoring thread 
	 * if the monitor is already stopped, it informs the calling function appropriately
	 * @author Tushar
	 * @param serviceURL
	 * @return ExecutionResponseVO
	 */
	...
	private boolean isServerMonitored(SchedulerVO schVO) {
		...
		if(rTask.getSchedulerState()
				&&isThreadRunning(schVO.getServiceName())) {
			return true;
		}else {
			return false;
		}
